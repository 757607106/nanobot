...
## 更新摘要
**所做更改**
- 新增配置示例文件章节，详细介绍 config.example.json 的完整配置模板
- 更新配置示例部分，增加实际的配置模板参考
- 增强配置最佳实践建议，包含示例文件的使用指导
- 完善配置模板的使用场景和修改建议
...
- 文档与示例：README.md、config.example.json
...
E["config.example.json<br/>完整配置模板"]
end
I --> L
I --> S
I --> P
...
### config.example.json 完整模板
Nanobot 提供了一个完整的配置示例文件 `config.example.json`，包含了所有可用的配置选项和默认值，为用户提供了完整的配置模板参考。

#### 示例文件结构
配置示例文件采用 JSON 格式，包含以下主要部分：

- **agents.defaults**：代理默认配置，包括工作区、模型、提供商、温度等
- **channels**：通道配置，包含发送进度、工具提示等全局设置
- **providers**：提供商配置，支持多种 AI 服务提供商
- **gateway**：网关配置，包括监听地址、端口和心跳设置
- **tools**：工具配置，包括网络工具、执行工具和 MCP 服务器

#### 配置模板使用指南
1. **复制示例文件**：将 `config.example.json` 复制为实际使用的配置文件
2. **修改敏感信息**：替换 API 密钥、令牌等敏感信息
3. **调整功能开关**：根据需要启用或禁用特定功能
4. **配置提供商**：添加所需的 AI 服务提供商配置
5. **设置通道参数**：配置各个聊天通道的具体参数
...
#### 配置最佳实践
- **安全性**：不要在配置文件中存储明文密码，优先使用环境变量
- **可维护性**：为不同环境准备不同的配置文件
- **版本控制**：将示例文件纳入版本控制，但排除实际配置文件
- **文档化**：为配置文件添加注释说明各个参数的作用
...
Example["config.example.json"] --> Schema
```
...
## 结论
Nanobot 的配置系统以 Pydantic 模型为核心，结合动态加载、路径派生与 Web 运行时服务，实现了灵活、可扩展且安全的配置管理。通过迁移机制与回退策略，系统在版本演进与异常情况下仍能保持稳定；多实例与环境隔离设计满足生产部署需求。新增的 `config.example.json` 配置示例文件为用户提供了完整的配置模板参考，简化了配置过程并提高了可维护性。建议在实际使用中遵循最小权限原则、优先使用环境变量、严格控制通道白名单，并定期备份配置文件。
...
### 配置示例文件使用指南
- **获取模板**：从项目根目录复制 `config.example.json` 到目标配置文件
- **修改配置**：根据实际需求修改各项配置参数
- **安全设置**：将敏感信息移动到环境变量或外部密钥管理
- **验证配置**：使用 `nanobot onboard` 命令验证配置的有效性
- **多实例配置**：为不同实例创建独立的配置文件副本

**章节来源**
- [config.example.json:1-92](file://config.example.json#L1-L92)
- [README.md:1088-1142](file://README.md#L1088-L1142)