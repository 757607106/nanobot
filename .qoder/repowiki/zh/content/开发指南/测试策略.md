...
- [web-ui/src/setup-route.test.tsx](file://web-ui/src/setup-route.test.tsx)
...
- [web-ui/src/test/renderApp.tsx](file://web-ui/src/test/renderApp.tsx)
- [web-ui/vite.config.ts](file://web-ui/vite.config.ts)
- [web-ui/src/test/setup.ts](file://web-ui/src/test/setup.ts)
</cite>
...
**更新** 新增了针对设置路由功能的单元测试套件，增强了设置向导路由的测试覆盖率，包括路由渲染、状态管理和请求去重机制的验证。

## 项目结构
Nanobot 的测试体系由后端 Python 单元/集成测试、前端 Vitest 单元测试与 Playwright 端到端测试、以及 CI 工作流三部分组成：
- 后端测试：基于 pytest，覆盖 Web API、服务层与业务流程（含 Mock 与临时配置）。
- 前端测试：基于 Vitest 和 Playwright，包含组件级单元测试与关键路径、可访问性端到端测试套件。
...
subgraph "前端测试"
V1["web-ui/vite.config.ts<br/>Vitest 配置"]
VT["web-ui/src/setup-route.test.tsx<br/>设置路由测试"]
...
- [web-ui/vite.config.ts:105-126](file://web-ui/vite.config.ts#L105-L126)
- [web-ui/src/setup-route.test.tsx:108-148](file://web-ui/src/setup-route.test.tsx#L108-L148)
...
- 测试框架与运行器
  ...
  - 前端 Vitest：组件级单元测试，支持 React 组件测试与 Mock。
  - 前端 Playwright：端到端测试，包含关键路径与可访问性测试套件。
- 测试环境
  - 后端：FastAPI TestClient 搭配临时配置与内存数据库；必要时通过临时目录隔离状态。
  - 前端 Vitest：jsdom 环境，支持 Ant Design 组件测试；Mock localStorage、sessionStorage 等浏览器 API。
  - 前端 Playwright：本地 uvicorn 服务器作为 E2E 服务端，Playwright 自动拉起 Chromium。
- Mock 与替身
  - 后端广泛使用 unittest.mock.AsyncMock、patch 替换外部依赖或内部服务。
  - 前端通过 vitest.hoisted 和 vi.mock 模拟 API 调用与全局对象。
- 测试数据管理
...
- [web-ui/vite.config.ts:105-126](file://web-ui/vite.config.ts#L105-L126)
- [web-ui/src/test/setup.ts:30-86](file://web-ui/src/test/setup.ts#L30-L86)
...
participant VT as "Vitest(前端单元)"
participant PW as "Playwright(前端E2E)"
...
### 前端单元测试：设置路由功能
- 设计原则
  - 使用 Vitest 进行组件级单元测试，支持 React 组件的渲染与交互验证。
  - 通过 vitest.hoisted 和 vi.mock 模拟 API 调用，确保测试隔离。
  - 使用 MemoryRouter 进行路由测试，验证设置向导路由的行为。
  - 通过 renderWithProviders 提供完整的上下文环境（AuthProvider、SetupProvider 等）。
- 关键流程
  - 设置路由渲染：验证 /setup 路由正确渲染欢迎标题和初始状态。
  - 请求去重：确保只发起一次认证、设置状态和配置请求，避免请求循环。
  - 状态管理：验证设置状态变化不会导致无限请求循环。
  - 组件渲染：验证设置向导的三个步骤（供应商、频道、Agent 默认值）正确显示。
...
participant SP as "SetupProvider"
participant API as "Mock API"
VT->>MR : 初始化路由 ['/setup']
MR->>AR : 渲染 AppRoutes
AR->>SP : SetupOnly 包装 SetupPage
SP->>API : getAuthStatus()
SP->>API : getSetupStatus()
SP->>API : getConfig()
SP->>API : getConfigMeta()
API-->>SP : 返回状态数据
SP-->>AR : 提供设置状态
AR-->>VT : 渲染 SetupPage
VT->>API : 验证请求次数
...
**图表来源**
- [web-ui/src/setup-route.test.tsx:126-147](file://web-ui/src/setup-route.test.tsx#L126-L147)
- [web-ui/src/App.tsx:166-195](file://web-ui/src/App.tsx#L166-L195)
- [web-ui/src/setup.tsx:39-96](file://web-ui/src/setup.tsx#L39-L96)
...
- 测试数据
  ...
  - 前端：Vitest 通过 vitest.hoisted 和 vi.mock 创建 Mock API 对象；E2E 服务器内置固定注册表与 MCP 服务器条目，确保一致性。
  - 前端单元测试：通过 makeConfig、makeConfigMeta、makeSetupStatus 辅助函数生成测试数据。
- Mock 策略
  - 后端：对第三方 SDK、HTTP 客户端、文件系统与外部进程进行替换，保证可控与可重复。
  - 前端：通过 vitest.hoisted 和 vi.mock 模拟网络行为与全局对象，避免真实外部依赖。

**章节来源**
- [web-ui/src/setup-route.test.tsx:23-106](file://web-ui/src/setup-route.test.tsx#L23-L106)
- [web-ui/src/test/setup.ts:30-86](file://web-ui/src/test/setup.ts#L30-L86)
...
- 测试框架与工具
  ...
  - 前端 Vitest：Vitest、@testing-library/react、jsdom 环境。
  - 前端 Playwright：Playwright、Axe-core、Vitest（用于组件级测试）。
- CI 工作流
...
VT["Vitest(web-ui)"] --> ST["setup-route.test.tsx"]
...
- 单元测试
  ...
  - Vitest 测试通过 jsdom 环境快速渲染组件，避免真实浏览器开销。
- 集成测试
...
## 结论
Nanobot 的测试策略以 pytest、Vitest 与 Playwright 为核心，结合 CI 工作流形成"关键路径 + 完整覆盖"的双轨测试体系。通过严格的 Mock 与测试数据管理、清晰的断言与可观测性配置，确保后端 API、服务层与前端 GUI 在变更中保持稳定。新增的设置路由单元测试进一步增强了前端路由功能的可靠性，特别是防止请求循环和确保正确的状态管理。建议持续优化用例粒度与并行度，在保证质量的前提下提升测试效率。
...
- 命名与分组
  ...
  - Vitest 测试使用 describe、it 进行分组，每个测试关注单一行为。
- 断言与期望
  - 明确断言点（状态码、响应体字段、会话 Cookie、文件存在性等）。
  - 对异步流程使用 await 与超时控制，避免 flaky。
  - Vitest 测试使用 waitFor 验证异步状态变化。
- 隔离与清理
  - 每个用例使用独立临时目录与配置，用例间不共享状态。
  - 用例结束清理临时文件与数据库，避免残留影响后续执行。
  - Vitest 通过 afterEach 和 cleanup 确保 DOM 清理。

### 测试覆盖率与报告
- 覆盖率
  - 建议后端覆盖率不低于 80%，关键路径不低于 90%；前端组件级测试覆盖率不低于 70%。
  - 新增设置路由测试确保路由相关功能达到 100% 覆盖。
- 报告
  - 后端：pytest 输出简洁报告；前端：Vitest 生成 HTML 报告与 Playwright HTML 报告与 artifacts 上传。
- CI 集成
