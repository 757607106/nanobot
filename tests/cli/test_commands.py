...
def test_onboard_fresh_install(mock_paths):
    ...
    assert (workspace_dir / "PROFILE.md").exists()
...
def test_agent_uses_default_config_when_no_workspace_or_config_flags(mock_agent_runtime):
    ...
    mock_agent_runtime["print_response"].assert_called_once_with(
        "mock-response", render_markdown=True, metadata=None,
    )
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanobot.cron.service.CronService", lambda _workspace, *args, **kwargs: object())
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    class _FakeCron:
        def __init__(self, workspace: Path, *args, **kwargs) -> None:
            seen["cron_workspace"] = workspace
    ...
    assert result.exit_code == 0
    assert seen["cron_workspace"] == config.workspace_path
    ...
    *,
    ...
    if session_manager is not None:
        monkeypatch.setattr(
            ...
            lambda workspace, *args, **kwargs: session_manager(workspace),
        )
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr(
        "nanobot.session.manager.SessionManager",
        lambda _workspace, *args, **kwargs: object(),
    )

    class _FakeCron:
        def __init__(self, _workspace: Path, *args, **kwargs) -> None:
            self.on_job = None
            seen["cron"] = self
