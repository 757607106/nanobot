        ...
        <PageHeader
          ...
          subtitle={(
            <Flex gap={8} wrap="wrap" align="center">
              ...
              <Tag color={entry.enabled ? 'success' : 'default'} bordered={false}>
                {entry.enabled ? '已启用' : '已停用'}
              </Tag>
              <Tag bordered={false}>
                {transportLabels[entry.transport] || entry.transport}
              </Tag>
            </Flex>
          )}
          actions={(
      ...
      <div style={{ paddingTop: 24, paddingBottom: 60 }}>
        ...
        >
          ...
          <Flex vertical gap={16} style={{ minWidth: 0 }}>
            ...
            <SectionCard
              title="运行概览"
              description="当前启用状态、探测结果与工具同步情况。"
            >
              <Descriptions
                size="small"
                column={1}
              ...
              >
                ...
                <Descriptions.Item label="工具数">
                  {entry.toolCountKnown ? `${entry.toolCount} 个` : '待测试'}
                </Descriptions.Item>
            ...
            <SectionCard
              title="可用工具"
              description="测试通过后会展示当前连接暴露出的工具列表。"
              action={<Tag>{entry.toolCountKnown ? `${entry.toolCount} 个` : '待测试'}</Tag>}
            >
