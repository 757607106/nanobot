...
>
> 2026-03-27 补充：关于 agent execution harness、workspace / sandbox、child-task 协议、middleware spine、以及多 Agent 执行内核的正式目标架构与实施进度，见 [`docs/web-platformization/agent-harness-blueprint.md`](./agent-harness-blueprint.md)。自本日期起，凡涉及 agent 执行内核的设计判断，均以该文档为准；本文件保留 control plane 与平台化总计划口径。
...
| 记忆体系 | 已有 session JSONL、`MEMORY.md / HISTORY.md`、token consolidation，以及 team shared memory / `agent_profile memory` / memory candidate 治理。 | 当前已覆盖 workspace memory、team shared memory、agent profile memory、team thread transcript 与 unified memory search；但底层 `MEMORY.md` 自动沉淀语义与显性 `memory policy` 仍未完全统一到同一 harness。 | `nanobot/session/manager.py`、`nanobot/agent/memory.py`、`nanobot/agent/loop.py`、`nanobot/platform/memory/*`、`nanobot/web/routers/memory.py`、`nanobot/web/runtime_services/teams.py`、`nanobot/web/runtime_services/agents.py`、`web-ui/src/pages/AgentsPage.tsx`、`web-ui/src/pages/MemoryAuditPage.tsx` |
...
| SaaS 多租户 | 已有 tenant-aware 鉴权、tenant-scoped API/service view 与部分执行边界。 | 当前 `PlatformInstance` 仍以单实例路径模型为主，tenant-aware workspace / artifact / run / knowledge / memory / channel resolution、`boundary audit`、独立 `channel audit` 控制面、artifact lifecycle governance 以及 tenant-aware child-task limit 已开始落地，但租户级对象模型、artifact retention/archival 策略与更完整隔离仍未完成。 |
...
| T60 | 设计并实现 tenant-aware 控制面边界与租户隔离基础 | 进行中 | 当前已落地 tenant-aware 鉴权、tenant-scoped service view、run lineage / artifact / knowledge / memory / channel resolution 的第一轮隔离，并已补齐 tenant-aware runs API、artifact governance、`artifact_written` / `channel_dispatch_resolved` provenance、`boundary-audit` 聚合、Runs 控制面展示、独立 `channel audit` store/API/页面、artifact lifecycle governance、artifact retention/archival policy、tenant 默认 retention policy、agent/team 模板级 retention policy、后台 retention sweep，以及 tenant-aware child-task limit；后续仍需继续补齐租户级对象模型与更完整执行边界。 |
