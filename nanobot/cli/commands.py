...
def gateway(
    ...
    from nanobot.cli.platform_runtime import build_cli_gateway_routing_runtime
    from nanobot.config.loader import get_config_path
    ...
    channels = ChannelManager(config, bus, routing_service=platform_runtime.routing_service)
    ...
    bindings_count = len(platform_runtime.channel_bindings.list_bindings(tenant_id="default"))
    if bindings_count > 0:
...
def web_ui(
    ...
    frontend: str = typer.Option(
        "auto",
        "--frontend",
        help="Frontend mode: auto (prefer Vite hot reload), static, or dev.",
    ...
    frontend_mode = frontend.lower()
    if frontend_mode not in {"auto", "static", "dev"}:
        console.print("[red]Invalid --frontend mode. Use auto, static, or dev.[/red]")
        raise typer.Exit(1)

    try:
        run_server(config=loaded, host=host, port=port, frontend_mode=frontend_mode)
...
def agent(
    ...
    async def _cli_progress(content: str, *, tool_hint: bool = False, **_: Any) -> None:
